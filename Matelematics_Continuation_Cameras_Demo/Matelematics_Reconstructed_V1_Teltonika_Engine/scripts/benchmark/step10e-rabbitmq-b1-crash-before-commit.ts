/** B1.2: kill a separate worker after delivery, before any DB write or ACK. */
...
async function crashWorker(messageId: string) {
  const connection = await amqp.connect(localRabbitUrl(), { timeout: 10000 });
  const channel = await connection.createChannel();
  await channel.prefetch(1);
  await channel.consume(topology.queue, msg => {
    if (!msg || msg.properties.messageId !== messageId || msg.fields.redelivered) {
      console.error('Unexpected first delivery; message retained without ACK');
      process.exit(43);
    }
    // The process exits with an open channel: RabbitMQ must requeue its unacked delivery.
    // No PostgreSQL connection exists in this child process.
    process.stdout.write('B1.2_DELIVERED\n', () => process.exit(42));
...
async function main() {
  ...
  let commits = 0, acks = 0, completed = false;
  try {
    await db.connect();
    ...
    const child = spawn(process.execPath, ['--import', 'tsx', process.argv[1], '--crash-worker', event.message_id],
    ...
    if (outcome.code !== 42 || !childOutput.includes('B1.2_DELIVERED'))
      throw new Error(`Crash worker failed: ${childError.slice(0, 300)} (exit ${outcome.code}, ${outcome.signal})`);
    deliveries++;
    const beforeRecovery = await db.query('SELECT count(*)::int AS n FROM b1.events WHERE run_id=$1::uuid', [runId]);
    if (beforeRecovery.rows[0].n !== 0) throw new Error('Write occurred before worker crash');
    ...
    if (!await persistThenAck(db, runId, event, () => { consumer!.ack(recovered); acks++; }))
      throw new Error('Expected exactly one new logical commit');
    commits++;
    ...
    if (rows.rows.length !== 1 || rows.rows[0].message_id !== event.message_id ||
        ...
        commits !== 1 || acks !== 1)
      throw new Error('B1.2 assertion failed; evidence retained');
    ...
    if (remaining.rows[0].n !== 0) throw new Error('B1.2 cleanup incomplete');
    completed = true;
    console.log(JSON.stringify({ event: 'step10e-rabbitmq-b1-crash-before-commit', result: 'PASS', runId,
      published, confirmed, deliveries, redeliveries, uniqueLogicalCommits: commits,
    ...
    if (!completed) console.error(JSON.stringify({ event: 'step10e-rabbitmq-b1-incomplete', runId,
      published, confirmed, deliveries, commits, acks, cleanup: 'Evidence retained for inspection' }));
  }
}
