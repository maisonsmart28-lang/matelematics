/** Local B1: worker exits after DB commit, before ACK; replay is idempotent. */
...
async function crashWorker(runId) {
  ...
  const db = new pg.Client({ connectionString: benchmarkDatabaseUrl(), ssl: false,
    ...
    application_name: 'matelematics_nats_b1_crash_worker_local' });
  await db.connect();
  ...
  if (identity.rows[0]?.db !== 'matelematics_b1' || identity.rows[0]?.username !== 'b1_benchmark')
    throw new Error('Unexpected crash worker database identity');
  // persistThenAck invokes this callback only after COMMIT; exit before sending an ACK.
  await persistThenAck(db, runId, event, () => {
    process.stdout.write('B1_AFTER_COMMIT_COMMITTED\n', () => process.exit(42));
  });
  await sleep(5000);
  throw new Error('Crash worker did not exit at commit boundary');
}
...
async function main() {
  ...
  const db = new pg.Client({ connectionString: benchmarkDatabaseUrl(), ssl: false,
    connectionTimeoutMillis: 10000, statement_timeout: 10000, query_timeout: 15000,
    application_name: 'matelematics_nats_b1_after_commit_local' });
  ...
  try {
    ...
    if (mainBefore.state.messages || deadBefore.state.messages || consumerBefore.num_pending ||
        consumerBefore.num_ack_pending ||
    ...
    if (code !== 42 || !stdout.includes('B1_AFTER_COMMIT_COMMITTED'))
      throw new Error(`Crash worker failed: ${stderr.slice(0, 250)}`);
    deliveries++;
    const rowsBeforeReplay = await db.query('SELECT message_id FROM b1.events WHERE run_id=$1::uuid', [runId]);
    if (rowsBeforeReplay.rows.length !== 1 || rowsBeforeReplay.rows[0].message_id !== event.message_id)
      throw new Error('Worker did not commit before simulated crash');
    ...
    if (await persistThenAck(db, runId, event, () => { acknowledgment = replay.ackAck({ timeout: 5000 }); }))
      throw new Error('Replay inserted an unexpected duplicate logical event');
    ...
    if (confirmed !== 1 || deliveries !== 2 || acks !== 1 || rows.rows.length !== 1 ||
      ...
      throw new Error('NATS B1 crash-after-commit assertion failed; evidence retained');
    ...
    completed = true;
    console.log(JSON.stringify({ event: 'step10e-nats-b1-crash-after-commit', result: 'PASS', runId,
      published: 1, confirmed, deliveries, redeliveries: 1, uniqueLogicalCommits: 1,
      duplicateDeliveriesDetected: 1, acks,
